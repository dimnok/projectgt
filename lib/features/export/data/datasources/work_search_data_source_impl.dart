...
class WorkSearchDataSourceImpl implements WorkSearchDataSource {
  ...
  Future<List<WorkSearchResult>> searchMaterials({
    ...
    try {
      ...
      if (startDate != null) {
        worksQuery = worksQuery.gte('date', startDate.toIso8601String().split('T')[0]);
      }
      if (endDate != null) {
        worksQuery = worksQuery.lte('date', endDate.toIso8601String().split('T')[0]);
      }
      ...
      final filteredWorkIds = worksResponse
          .map((work) => work['id'] as String)
          .toSet()
          .toList();
      ...
      for (final workItem in workItemsResponse) {
        ...
        if (!itemName.toLowerCase().contains(searchQuery.toLowerCase())) continue;
        ...
        results.add(WorkSearchResult(
          ...
          materialName: workItem['name'] as String? ?? '', // Используем название работы как "материал"
