...
/// Выбранный период дат для модуля выгрузки
final exportDateRangeProvider = StateProvider<DateTimeRange?>((ref) => null);

/// Действие в AppBar: кнопка с календарём, открывающая компактный выбор диапазона дат
class ExportDateRangeAction extends ConsumerWidget {
  /// Конструктор действия выбора периода дат.
  const ExportDateRangeAction({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final range = ref.watch(exportDateRangeProvider);
...
class _CompactDateRangePickerState
  ...
  void initState() {
    super.initState();
    _today = DateTime.now();
    final current = ref.read(exportDateRangeProvider);
                ...
                children: [
                  TextButton(
                    onPressed: () {
                      ...
                      ref.read(exportDateRangeProvider.notifier).state = null;
                      // Применяем дефолтный период (текущий месяц) сразу к таблице, не трогая остальные фильтры
                    ...
                    onPressed: () {
                      if (_start != null && _end != null) {
                        ...
                        ref.read(exportDateRangeProvider.notifier).state =
                            range;
                        // Пробрасываем выбранный период в провайдер фильтров выгрузки
                        ref.read(exportFilterProvider.notifier).setPeriod(
                              DateTime(
                                  _start!.year, _start!.month, _start!.day),
                              DateTime(_end!.year, _end!.month, _end!.day),
                            );
                        // Сразу применяем фильтр к таблице
                        final st = ref.read(exportFilterProvider);
                        final filter = ExportFilter(
                          objectIds: st.objectIds,
                          contractIds: st.contractIds,
