...
class WorkSearchDateRangeAction extends ConsumerWidget {
  ...
  const WorkSearchDateRangeAction({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final range = ref.watch(workSearchDateRangeProvider);
      ...
      await showMenu(
        ...
        items: const [
          PopupMenuItem(
            ...
            child: _WorkSearchCompactDateRangePicker(),
          ),
        ],
      );
    }
...
/// Компактный выбор периода для поиска работ
class _WorkSearchCompactDateRangePicker extends ConsumerStatefulWidget {
  const _WorkSearchCompactDateRangePicker();
  @override
  ConsumerState<_WorkSearchCompactDateRangePicker> createState() =>
      _WorkSearchCompactDateRangePickerState();
}

class _WorkSearchCompactDateRangePickerState
    extends ConsumerState<_WorkSearchCompactDateRangePicker> {
  ...
  void initState() {
    super.initState();
    _today = DateTime.now();
    final current = ref.read(workSearchDateRangeProvider);
                ...
                children: [
                  TextButton(
                    onPressed: () {
                      ...
                      ref.read(workSearchDateRangeProvider.notifier).state =
                          null;

                      // Перезагружаем поиск БЕЗ фильтра по датам
                      final searchNotifier =
                          ref.read(workSearchProvider.notifier);
                      ...
                      searchNotifier.searchMaterials(
                        objectId: selectedObjectId,
                        startDate: null,
                        endDate: null,
                        searchQuery:
                            searchQuery.trim().isEmpty ? null : searchQuery,
                  ...
                  FilledButton(
                    onPressed: () {
                      if (_start != null && _end != null) {
                        ...
                        // Перезагружаем поиск с новым периодом
                        final searchNotifier =
                            ref.read(workSearchProvider.notifier);
                        final selectedObjectId =
                            ref.read(exportSelectedObjectIdProvider);
...
class _CompactMonthGridState extends State<_CompactMonthGrid> {
  ...
  bool _inRange(DateTime d) {
    ...
    if (e == null) return _isSameDay(d, s);
  ...
  Widget build(BuildContext context) {
    ...
    final leading = (firstWeekday + 6) % 7; // понедельник = 0
