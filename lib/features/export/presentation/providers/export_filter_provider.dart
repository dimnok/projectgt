...
class ExportFilterState {
  ...
  }) : dateFrom = dateFrom ?? DateTime(DateTime.now().year, DateTime.now().month, 1),
       dateTo = dateTo ?? DateTime(DateTime.now().year, DateTime.now().month + 1, 0);
...
class ExportFilterNotifier extends StateNotifier<ExportFilterState> {
  ...
  void setContractFilter(List<String> ids) => state = state.copyWith(contractIds: ids);
  
  /// Установить фильтр по системам.
  void setSystemFilter(List<String> systems) => state = state.copyWith(systems: systems);
  
  /// Установить фильтр по подсистемам.
  void setSubsystemFilter(List<String> subsystems) => state = state.copyWith(subsystems: subsystems);
  ...
  void setDateFrom(DateTime dateFrom) => state = state.copyWith(dateFrom: dateFrom);
...
final exportFilterProvider = StateNotifierProvider<ExportFilterNotifier, ExportFilterState>((ref) {
...
final availableObjectsForExportProvider = Provider<List<dynamic>>((ref) {
  ...
  final filteredEstimates = estimateState.estimates.where((estimate) {
    final byContract = filter.contractIds.isEmpty || filter.contractIds.contains(estimate.contractId);
    final bySystem = filter.systems.isEmpty || filter.systems.contains(estimate.system);
    final bySubsystem = filter.subsystems.isEmpty || filter.subsystems.contains(estimate.subsystem);
...
final availableContractsForExportProvider = Provider<List<dynamic>>((ref) {
  ...
  final filteredEstimates = estimateState.estimates.where((estimate) {
    final byObject = filter.objectIds.isEmpty || filter.objectIds.contains(estimate.objectId);
    final bySystem = filter.systems.isEmpty || filter.systems.contains(estimate.system);
    final bySubsystem = filter.subsystems.isEmpty || filter.subsystems.contains(estimate.subsystem);
