...
final payrollWorkHoursProvider = FutureProvider<List<dynamic>>((ref) async {
  ...
  final client = ref.watch(supabaseClientProvider);
    ...
    final workHours = response.map<dynamic>((record) {
      final works = record['works'] as Map<String, dynamic>?;
      final employee = record['employees'] as Map<String, dynamic>?;
      
      if (works == null || record['employee_id'] == null || record['hours'] == null || works['date'] == null) {
  ...
  try {
    ...
    final filteredEntries = workHours.where((entry) {
      ...
      if (positionNames.isNotEmpty) {
        final employee = filterState.employees.firstWhereOrNull((e) => e.id == entry.employeeId);
        byPosition = employee != null && employee.position != null && positionNames.contains(employee.position);
      }
...
final payrollPayoutsByMonthProvider = FutureProvider.family<List<PayrollPayoutModel>, DateTime>((ref, month) async {
  final client = ref.watch(supabaseClientProvider);
...
final filteredPayrollPayoutsProvider = FutureProvider<List<PayrollPayoutModel>>((ref) async {
  try {
