...
class PayrollFilterNotifier extends StateNotifier<PayrollFilterState> {
  ...
  void updateDataFromProviders() {
    try {
      ...
      if (employeeState.employees.isNotEmpty ||
          objectState.objects.isNotEmpty) {
  ...
  void setPositionFilter(List<String> positions) =>
      state = state.copyWith(positionNames: positions);
...
final availableEmployeesForPeriodProvider = Provider<List<dynamic>>((ref) {
  ...
  final filteredEntries = periodEntries.where((entry) {
    final byObject =
        filter.objectIds.isEmpty || filter.objectIds.contains(entry.objectId);

    // Для фильтрации по должности получаем данные из employees
    bool byPosition = true;
    if (filter.positionNames.isNotEmpty) {
      final employee =
          filter.employees.firstWhereOrNull((e) => e.id == entry.employeeId);
      byPosition = employee != null &&
          employee.position != null &&
          filter.positionNames.contains(employee.position);
    }
...
final availableObjectsForPeriodProvider = Provider<List<dynamic>>((ref) {
  ...
  final filteredEntries = periodEntries.where((entry) {
    final byEmployee = filter.employeeIds.isEmpty ||
        filter.employeeIds.contains(entry.employeeId);
  ...
  final objectIds = filteredEntries
      .map((e) => e.objectId)
