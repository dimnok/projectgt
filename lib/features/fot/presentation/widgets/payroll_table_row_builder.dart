...
class PayrollTableRowBuilder {
  ...
  static List<DataRow> buildDataRows({
    ...
    final sortedEmployeeKeys = _sortEmployeeKeys(groupedPayrolls.keys.toList(), employees);
    ...
    rows.add(_buildTotalRow(totals, theme, isMobile, isTablet, isDesktop, groupedPayrolls, aggregatedBalance));
    
    return rows;
  }
  
  /// Создаёт отображение employeeId -> position
  static Map<String, String> _buildEmployeePositionsMap(List<dynamic> timesheetEntries) {
  ...
  static List<String?> _sortEmployeeKeys(List<String?> keys, List<dynamic> employees) {
  ...
  static EmployeeInfo _getEmployeeInfo(
    ...
    if (employeeId != null && employeeId != 'unknown') {
      final employee = employees.firstWhereOrNull((emp) => emp.id == employeeId);
      if (employee != null) {
        employeeName = '${employee.lastName} ${employee.firstName} ${employee.middleName ?? ''}';
        employeePosition = employee.position ?? employeePositions[employeeId] ?? '';
      } else {
  ...
  static List<DataCell> _buildRowCells({
    ...
    if (isDesktop) {
      cells.addAll(_buildDesktopCells(payroll, payoutsByEmployee, aggregatedBalance, theme));
    } else if (isTablet) {
      cells.addAll(_buildTabletCells(payroll, payoutsByEmployee, aggregatedBalance, theme, isMobile));
    } else {
      cells.addAll(_buildMobileCells(payroll, payoutsByEmployee, aggregatedBalance, theme));
    }
  ...
  static List<DataCell> _buildDesktopCells(
    ...
    return [
      ...
      PayrollTableCellBuilder.buildPayoutCell(payoutsByEmployee[payroll.employeeId], theme),
      PayrollTableCellBuilder.buildBalanceCell(aggregatedBalance[payroll.employeeId ?? ''] ?? 0, theme),
    ];
  }
  ...
  static List<DataCell> _buildTabletCells(
    ...
    if (!isMobile) {
      cells.add(PayrollTableCellBuilder.buildBonusCell(payroll.bonusesTotal, theme));
    }
  ...
  static DataRow _buildTotalRow(
    ...
    if (isDesktop) {
      totalCells.addAll(_buildDesktopTotalCells(totals, theme, groupedPayrolls, aggregatedBalance));
    } else if (isTablet) {
      totalCells.addAll(_buildTabletTotalCells(totals, theme, isMobile, groupedPayrolls, aggregatedBalance));
    } else {
      totalCells.addAll(_buildMobileTotalCells(totals, theme, groupedPayrolls, aggregatedBalance));
    }
  ...
  static List<DataCell> _buildDesktopTotalCells(
    ...
    return [
      ...
      PayrollTableCellBuilder.buildCurrencyCell(totals.base, fontWeight: FontWeight.bold),
      PayrollTableCellBuilder.buildBonusCell(totals.bonus, theme),
      PayrollTableCellBuilder.buildPenaltyCell(totals.penalty, theme),
      PayrollTableCellBuilder.buildCurrencyCell(totals.trip, fontWeight: FontWeight.bold),
      PayrollTableCellBuilder.buildTotalCell(totals.amount, theme),
      PayrollTableCellBuilder.buildTotalCell(totals.payout, theme, backgroundColor: Colors.blue[50]),
...
/// отработанные часы, базовая сумма, премии, штрафы, командировочные, выплаты, баланс.
class PayrollTotals {
  /// Суммарное количество отработанных часов по всем сотрудникам.
  double hours = 0;

  /// Суммарная базовая сумма начислений (без премий, штрафов и командировочных).
  double base = 0;
  ...
  /// Суммарная сумма командировочных выплат по всем сотрудникам.
  double trip = 0;
