-- При фильтре объектов не включать в ведомость сотрудников только из-за выплаты.
-- У выплат нет object_id: иначе срез «Офис» показывает почти весь штат за месяц выплат.
...
BEGIN
  ...
  bonus_calc AS (
    ...
    WHERE EXTRACT(YEAR FROM pb.date) = p_year
      ...
      AND (p_object_ids IS NULL OR pb.object_id = ANY(p_object_ids))
  ...
  penalty_calc AS (
    ...
    WHERE EXTRACT(YEAR FROM pp.date) = p_year
      ...
      AND (p_object_ids IS NULL OR pp.object_id = ANY(p_object_ids))
  ...
  WHERE (
      bc.emp_id IS NOT NULL
      OR bonus.emp_id IS NOT NULL
      OR pen.emp_id IS NOT NULL
      OR (p_object_ids IS NULL AND pout.emp_id IS NOT NULL)
    )
...
COMMENT ON FUNCTION public.calculate_payroll_for_month(integer, integer, uuid[], uuid) IS
  'Ведомость ФОТ за месяц. При p_object_ids: часы, премии и штрафы выбранных объектов; выплаты в состав строк не входят.';
