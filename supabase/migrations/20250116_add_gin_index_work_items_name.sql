...
'GIN триграммный индекс для оптимизации поиска по наименованию работ (name) с использованием ILIKE. ' ||
'Позволяет быстро находить записи даже при поиске с % в начале паттерна. ' ||
'Оптимизирует поиск внутри объекта (50-200k записей), так как поиск всегда выполняется после фильтрации по объекту.';
