-- Фильтр объектов в ФОТ: премии и штрафы без object_id не входят в срез объекта.
-- Как у часов: p_object_ids IS NULL → все; иначе только совпадение object_id.
-- Текущие данные: object_id заполнен у всех премий и штрафов — суммы не меняются.
...
COMMENT ON FUNCTION public.calculate_payroll_for_month(integer, integer, uuid[], uuid) IS
  'Ведомость ФОТ за месяц. При p_object_ids премии и штрафы без object_id в срез не входят.';
