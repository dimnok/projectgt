-- Сводка подрядчиков: добавление фактических сумм по закрытым сменам (work_items × estimates × works).
-- fact_own — сумма total строк без подрядчика; fact_sub_revenue — с total суба; fact_sub_cost — quantity × unit_price из estimate_contractor_prices.

DROP FUNCTION IF EXISTS public.get_subcontractor_margin_dashboard(uuid);
