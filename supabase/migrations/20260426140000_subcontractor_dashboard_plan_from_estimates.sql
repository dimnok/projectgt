-- Сводка «Подрядчики»: все группы смет (estimates) по объекту/договору/названию.
-- Наша сумма = SUM(estimates.total); суб = сумма по расценкам (кол-во × unit_price).
-- Не зависит от work_items.
...
BEGIN
  ...
  ranked_ecp AS (
    SELECT
      ...
      ld.estimate_title,
      ecp.contractor_id,
      (COALESCE(ecp.contractor_quantity::numeric, ld.quantity::numeric) *
      (ecp.unit_price::numeric)) AS line_cost
    FROM line_data ld
  ...
  agg AS (
    SELECT
      ...
      COALESCE(SUM(ld.sub_line), 0::numeric) AS sub_amount,
  ...
  SELECT
    ...
    t.contractor_id,
    a.our_amount,
    COALESCE(a.sub_amount, 0::numeric)::numeric AS subcontractor_planned_amount,
    a.unpriced_lines
  FROM agg a
  LEFT JOIN top_c t
...
COMMENT ON FUNCTION public.get_subcontractor_margin_dashboard(uuid) IS
  'Плановая сводка по сметам: наша сумма (SUM estimates.total), суб (расценки подрядчика), без work_items.';
