-- Сводка: одна строка на (объект, договор, название сметы, подрядчика).
-- Если на позиции сметы несколько расценок, «нашу» сумму по строке делим
-- пропорционально part_sub (как в объёмах суба).
...
BEGIN
  ...
  line_alloc AS (
    SELECT
      ...
      es.contractor_id,
      (e.total::numeric * (es.part_sub / NULLIF(lst.s_tot, 0::numeric)))::numeric
        AS our_alloc,
  ...
  by_contractor AS (
    SELECT
      ...
      la.estimate_title,
      la.contractor_id,
      COALESCE(SUM(la.our_alloc), 0::numeric) AS o_amt,
...
COMMENT ON FUNCTION public.get_subcontractor_margin_dashboard(uuid) IS
  'Сводка по сметам: строка на подрядчика; наша сумма распределена пропорционально субу на позиции; + строки без расценок.';
