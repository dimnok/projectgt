-- Сводка по сменам: работы с подрядчиком (fact work_items) × плановая стоимость
-- по расценке (estimate_contractor_prices) для оценки маржи.
-- RLS: SECURITY INVOKER — действуют политики на work_items / works / estimates.
...
COMMENT ON FUNCTION public.get_subcontractor_margin_dashboard(uuid) IS
  'Сводка по факт-работам с подрядчиком: наша сумма (work_items.total) и плановая стоимость подрядчика (кол-во × unit_price из estimate_contractor_prices).';

GRANT EXECUTE ON FUNCTION public.get_subcontractor_margin_dashboard(uuid) TO authenticated;
