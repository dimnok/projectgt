-- Финансы в профиле: только свой сотрудник и активная компания.
-- Без FIFO и без выдачи ведомости всей компании на клиент.
-- Начисления месяца = та же формула, что calculate_payroll_for_month (без фильтра объектов).
-- Выплаты месяца = сумма payroll_payout по календарной дате выплаты.
-- Остаток = начисления за всё время − все выплаты (как calculate_single_employee_balance).
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(SUM(ah.work_hours * COALESCE((
      ...
      WHERE er.employee_id = v_employee_id
        ...
        AND er.company_id = v_company_id
  ...
  SELECT COALESCE(SUM(COALESCE((
    SELECT btr.rate
    FROM public.business_trip_rates btr
    WHERE btr.object_id = ah.obj_id
      AND btr.company_id = v_company_id
  ...
  INTO v_bonus
  FROM public.payroll_bonus pb
  WHERE pb.employee_id = v_employee_id
  ...
  INTO v_penalty
  FROM public.payroll_penalty pp
  WHERE pp.employee_id = v_employee_id
  ...
  INTO v_payout_month
  FROM public.payroll_payout po
  WHERE po.employee_id = v_employee_id
    AND po.company_id = v_company_id
    AND po.payout_date >= v_start
    AND po.payout_date < v_end;
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'date', pb.date,
    'amount', pb.amount,
    'reason', COALESCE(pb.reason, '')
  ) ORDER BY pb.date DESC), '[]'::jsonb)
  INTO v_bonuses
  FROM public.payroll_bonus pb
  WHERE pb.employee_id = v_employee_id
    AND pb.company_id = v_company_id;
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'date', pp.date,
    'amount', pp.amount,
    'reason', COALESCE(pp.reason, '')
  ) ORDER BY pp.date DESC), '[]'::jsonb)
  INTO v_penalties
  FROM public.payroll_penalty pp
  WHERE pp.employee_id = v_employee_id
    AND pp.company_id = v_company_id;
  ...
  INTO v_hours_by_date
  FROM (
    SELECT ah.work_date, SUM(ah.work_hours) AS hours_sum
    FROM all_hours ah
    GROUP BY ah.work_date
  ) d;
  ...
  RETURN jsonb_build_object(
    ...
    'hours', v_hours,
    'base_salary', v_base,
    'business_trip_total', v_trip,
    'bonuses_total', v_bonus,
    'penalties_total', v_penalty,
    'net_salary', v_net,
    'payouts_month', v_payout_month,
    'month_delta', v_net - v_payout_month,
    'balance', COALESCE(v_balance, 0),
    'bonuses', v_bonuses,
    'penalties', v_penalties,
    'payouts', v_payouts,
    'hours_by_date', v_hours_by_date
  );
END;
$$;

ALTER FUNCTION public.get_my_profile_finance(integer, integer) OWNER TO postgres;

REVOKE ALL ON FUNCTION public.get_my_profile_finance(integer, integer) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.get_my_profile_finance(integer, integer) FROM anon;
GRANT EXECUTE ON FUNCTION public.get_my_profile_finance(integer, integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_my_profile_finance(integer, integer) TO service_role;

COMMENT ON FUNCTION public.get_my_profile_finance(integer, integer) IS
  'Финансы текущего пользователя. Суммы месяца — ставка, суточные, премии, штрафы, итог. Списки премий и штрафов — за всё время.';
