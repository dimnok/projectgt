...
- `OPENROUTER_API_KEY` — используется ещё функцией `get-daily-tip` (ежедневный совет на главной). **НЕ удалять**, пока `get-daily-tip` не убран/переписан.
...
- [ ] (Опционально) Удалить секрет `OPENROUTER_API_KEY`, если `get-daily-tip` тоже будет убран.
