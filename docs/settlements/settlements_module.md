...
**Изменения:**
- **Фильтры реестра:** контрагент, объект, договор (каскадная взаимосвязь), тип, статус оплаты, поиск; кнопка «Сбросить»; клиентская логика в `SettlementsListFilters` / `SettlementsFilterOptionsBuilder`.
...
- Автоматический статус оплаты, фильтры (контрагент, объект, договор, тип, оплата, поиск); итоги «К оплате / Оплачено / Остаток» — в подвале таблицы реестра.
...
| Виджет | Назначение |
|--------|------------|
| `SettlementsFiltersToolbar` | Поиск; каскадные фильтры **контрагент / объект / договор**; тип; статус оплаты; «Сбросить» (при активных фильтрах); «Новый». Горизонтальная прокрутка при нехватке ширины |
...
### Фильтры реестра (`SettlementsListScreen` + `SettlementsListFilters`)

Клиентская фильтрация загруженного списка (`settlementListProvider`), без дополнительных запросов к БД.

| Фильтр | Поведение |
|--------|-----------|
| Поиск | Номер счёта, акт, договор, контрагент, объект |
| Контрагент | Сужает списки объектов и договоров |
| Объект | Сужает списки контрагентов и договоров; сброс объекта также сбрасывает договор |
| Договор | При выборе подставляет связанные контрагента и объект |
| Тип / Оплата | Независимые enum-фильтры |
| Сбросить | Очищает все фильтры разом (видна при `hasActive`) |

Списки опций строятся из операций компании (`SettlementsFilterOptionsBuilder`). Пункты «Все …» в выпадающих меню сбрасывают фильтр через `PopupMenuItem.onTap` (обход ограничения Flutter: `value: null` не вызывает `onSelected`).
...
| `settlements_filter_options.dart` | `SettlementsFilterOption`, сборка уникальных опций из операций |
| `settlements_list_filters.dart` | `SettlementsListFilters` — состояние, каскад, `apply()`, `hasActive`, `cleared()` |
...
### Реализовано
- ✅ Каскадные фильтры реестра: контрагент, объект, договор + сброс
