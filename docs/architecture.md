...
- `features/purchase_requests/` — заявки на закупку. Workflow: черновик → согласование → счета → оплата → получение. После отправки откат в черновик запрещён; вернуть на доработку может только согласующий очереди. Подробно: `docs/purchase_requests/purchase_requests_module.md`.
