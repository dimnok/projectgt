...
- **Web: банковские счета, 02.09.2026:**
    * В открытой карточке контрагента на `/contractors` тот же раздел, что во Flutter: список, добавление, изменение, удаление.
    * Поля: банк, БИК, номер счёта, корр. счёт, «Счёт по умолчанию». `bank_city` показывается, в форму не входит и при сохранении не затирается.
    * Основной счёт один: при установке `is_primary` остальные счета этого контрагента сбрасываются.
- **Web: поиск по ИНН, 02.09.2026:**
    * Форма контрагента вызывает Edge Function `dadata-proxy` (тот же путь, что Flutter `searchCompanyByInn`).
    * В поля подставляются названия, КПП, ОГРН, ОКПО, адреса, директор, телефон, email.
    * ОКВЭД из ответа (`[код] название`) сохраняется как код; название берётся из справочника веба.
- **Уникальный ИНН, 02.09.2026:**
    * В рамках компании ИНН уникален (цифры, без учёта пробелов). Индекс `contractors_company_id_inn_digits_uidx`.
    * Web: форма не даёт сохранить дубль и показывает имя существующего контрагента.
...
- Web (`react_app`): тот же раздел в открытой карточке — список, форма и удаление счёта.
...
| inn | text | ИНН (10 или 12 цифр). Уникален в компании по цифрам |
| kpp | text | КПП |
| ogrn | text | ОГРН |
| okpo | text | ОКПО |
...
- **Индексы:** `contractors_company_id_inn_digits_uidx` — уникальный ИНН в рамках компании (только цифры, пустой ИНН не участвует).
...
- **Уникальный ИНН:** в одной компании один набор цифр ИНН. Web проверяет список до сохранения и показывает имя существующего контрагента. База отклоняет дубль (в том числе из Flutter).
- **ОКВЭД (`activity_description`):**
    ...
    - Web при заполнении по ИНН извлекает код из ответа `dadata-proxy` и при сохранении нормализует известные форматы к коду.
- **Адаптивность UI:** 
...
- **Банковские счета (Web):** раздел в открытой карточке `react_app`. Счета запрашиваются только для раскрытой карточки. Форма совпадает с Flutter, без поля города. При сохранении `bank_city` не отправляется, чтобы не стереть значение из базы.
...
- **Web:** `react_app` вызывает ту же `dadata-proxy` из формы контрагента. Код ОКВЭД из ответа кладётся в поле; название — из `react_app/src/lib/okved/`.
...
- 🟢 Web: поиск по ИНН через DaData (`dadata-proxy`).
- 🟢 Web: банковские счета в карточке контрагента.
