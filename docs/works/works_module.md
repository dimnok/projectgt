...
**Дата актуализации:** 10 января 2026 года (Аудит модуля: унификация логики групп месяцев, внедрение стандартных модальных виджетов `MobileBottomSheetContent` и `DesktopDialogContent`, переход на `GTTextField`, оптимизация производительности)
...
- ✅ **Стандартные модальные окна:** Использование `MobileBottomSheetContent` и `DesktopDialogContent` для всех форм модуля (Создание смены, Часы сотрудника, Новый материал).
...
- `WorkPlanDetailsScreen` — экран деталей плана работ.
- `WorkDetailsPanel` — центральный компонент управления данными смены (табы: Данные, Работы, Сотрудники).
- `MonthDetailsPanel` — панель статистики за месяц с графиками и KPI (оптимизирована через `RepaintBoundary`).
- `MobileBottomSheetContent` / `DesktopDialogContent` — базовые обертки для всех модальных окон модуля.
- `GTTextField` — используется для всех полей ввода, включая компактные инлайн-редакторы (Design System).
...
- `monthGroupsProvider` — управление группами смен (использует `MonthGroupController`).
- `workPlanMonthGroupsProvider` — управление группами планов (вынесен в модуль `work_plans`).
...
2.  **Валидация при закрытии:** 
    - Наличие хотя бы одной работы и одного сотрудника.
    - Все объемы и часы должны быть > 0.
    - Наличие вечерней фотографии (фотофиксация результата).
3.  **Расчеты:** Суммы по работам (`total`) рассчитываются на стороне клиента и дублируются в `works.total_amount` для быстрой загрузки списков.
4.  **Удаление смен:** 
    - **Владелец компании (Owner):** имеет полное право на удаление любых смен.
    - **Обычный пользователь:** может удалять только свои открытые смены.
...
## Roadmap
- ✅ **Завершено:** Рефакторинг архитектуры и устранение дублирования логики групп месяцев.
- ✅ **Завершено:** Переход на `GTTextField` и системные цвета во всем модуле.
...
- 🟡 **Планы:** Интеграция с модулем Складов для списания материалов по факту выполнения работ.
