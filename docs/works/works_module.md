...
**Дата актуализации:** 19 сентября 2026 года — фото смены: права и замена снимка.
- **Замена фото удаляет прежний файл.** `PhotoService.uploadPhotoBytes` сначала загружает новый снимок, затем удаляет прежний того же типа (утренний/вечерний) в папке даты смены. Старый файл удаляется только после успешной загрузки нового — при сбое загрузки фото смены не теряется. Раньше очистка смотрела в папку объекта `{object_id}/`, а файлы лежат глубже — `{object_id}/{ДД-ММ-ГГГГ}/`, поэтому прежние снимки оставались мусором в хранилище.
- **Дата смены при замене.** Замена снимка (`work_photo_view`) передаёт `workDate` — фото кладётся в папку своей смены, а не в сегодняшнюю.
- **Кнопка «Заменить фото».** Блок валидации смены показывает кнопку и при уже загруженном вечернем фото (`WorksStrings.replacePhotoBtn`); диалог даёт «Удалить», «Сделать фото», «Выбрать из галереи». Раньше кнопка была только когда фото ещё нет — исправить снимок можно было лишь через полноэкранную галерею.
- **Владелец компании.** В полноэкранной галерее кнопка замены доступна владельцу компании — как в блоке валидации и как разрешает база (`check_work_editable`).
- **БД:** миграция `20260919180000_works_storage_policies.sql` — политики Storage для bucket `works` сужены: загрузка, изменение и удаление только по объектам своей компании и с правом «работы» (создание или изменение для загрузки, изменение или удаление для удаления). Было: любой авторизованный пользователь мог загружать и удалять любые фото. Чтение осталось публичным (bucket публичный).

**Дата актуализации:** 05 сентября 2026 года — `check_work_editable`: роль «Супер-админ» (`is_super_admin`) может обновлять и удалять закрытые смены, как владелец компании. Миграция `20260905200000_super_admin_check_work_editable.sql`. Веб: повторное открытие и удаление смены. Приложение (Flutter UI удаления) не менялось — там по-прежнему владелец / автор открытой смены.
...
6. **Удаление:** Owner — любые; супер-админ (`is_super_admin`) — любые (после 05.09.2026, `check_work_editable`); пользователь — свои open + RBAC `works.delete` / `work_plans.delete`. Веб показывает удаление и повторное открытие только супер-админу.
