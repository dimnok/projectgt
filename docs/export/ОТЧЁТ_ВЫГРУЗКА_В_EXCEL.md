# 📊 ОТЧЁТ: ДЕТАЛЬНЫЙ АНАЛИЗ ВЫГРУЗКИ В EXCEL
...
**⚠️ ВАЖНО:** Дата **НЕ учитывается** в группировке! Записи с разных дат, но с одинаковыми остальными параметрами будут объединены.
...
#### **Пример 2: Объединение записей с разных дат** ⭐ НОВОЕ
```
Исходные записи из БД:
...
2. **Опция "Объединять одинаковые позиции"** (`aggregate: bool`)
...
3. **Цена:** проверяется консистентность
   ```dart
...
| **Секция** | ✅ Учитывается | ❌ НЕ учитывается |
...
- **Цель:** Убрать дубликаты в рамках одной даты/секции/этажа
- **Ключ:** 11 полей
- **Результат:** Оптимизация данных для UI

#### **Вторичная (опциональная):**
- **Где:** `ExportService.exportToExcel()`
- **Когда:** При экспорте, если пользователь включил опцию
- **Цель:** Агрегировать данные по работам без детализации по датам/секциям
- **Ключ:** 6 полей
- **Результат:** Сводный отчёт (объединение работ за весь период)

### **2. Параметры объединения:**

**Первичная группировка (10 полей):** ⭐ ИЗМЕНЕНО
1. Объект
2. Договор
3. Система
4. Подсистема
