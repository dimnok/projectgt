# Главный экран: «Календарь смен» (Heatmap)

## Назначение
- Показать распределение выручки по дням текущего месяца в компактном виде (heatmap) на главном экране.
- Быстро выявлять дни с максимальной суммой и дни без смен.
- Быстрый переход к деталям по выбранной дате (оборотная сторона карточки с разбиением по объектам и системам).

## Расположение
- Экран: `features/home/presentation/screens/home_screen.dart`
- Виджеты календаря и flip-анимации: `features/home/presentation/widgets/shifts_calendar_widgets.dart`
- Виджет прогресса по договору: `features/home/presentation/widgets/contract_progress_widget.dart`

## Источник данных
- ⚠️ **Функциональность обновлена:** модуль выгрузки был удален.
- Текущая версия: данные берутся из провайдеров работ и объектов.
- На момент последнего обновления: модуль выгрузки содержал `exportProvider`, но эта функциональность была упрощена.

## Логика агрегации
- Для heatmap используются данные из модуля работ (work_items, works).
- Фильтруются записи текущего месяца, далее:
  - Суммирование по дню: `sumByDate[date] += (report.total ?? 0)`.
  - Подсчитывается `maxValue` по всем датам месяца для цветового акцента.
- Ключевая правка: группировка записей учитывает дату (по дню), что исключает «слипание» сумм разных дат и гарантирует корректные суммы.

## Отрисовка
- Сетка месяца (Пн..Вс) формируется по текущему месяцу с префиксными пустыми ячейками.
- Цвет ячейки:
  - 0 — мягкий красный (`_softRed`).
  - максимум месяца — зелёный (`_whatsappGreen`).
  - прочие значения — синий (`_telegramBlue`).
- Ячейка — `AnimatedContainer` (~14×14); при наведении показывается `Tooltip`: `DD.MM.YYYY — ₽ сумма`.

## Интерэктивность
- Mobile/Tablet: Тап по дате переворачивает карточку (flip) и показывает оборотную сторону с деталями по выбранной дате.
  - Оборотная сторона: вверху дата (слева) и кнопка возврата в календарь (справа, иконка `swap_horiz`).
  - Контент: список по объектам (название объекта и сумма), под каждым объектом — список его систем в виде строк «• название системы — сумма».
- Desktop (ширина ≥ 1100): календарь отображается без flip, в центральной колонке под карточкой «Сводка смен»; в правой колонке — карточка прогресса по договору.

## Интеграции
- Модуль работ: источник данных для heatmap и карточки прогресса.
- Модуль заявок на закупку: блок «Нужно согласовать» (`HomePurchaseRequestsApprovalWidget`) — только заявки, где текущий пользователь согласующий (`approval` / `invoice_approval` + соответствующие права). Если таких нет, блок скрыт. Нажатие открывает заявку (`/purchase_requests?requestId=`).
- Внешний вид блока: красный контур и полоса слева, значок восклицания, бейдж с числом. Цвет: `#DC2626` (светлая тема), `#F87171` (тёмная). Это сигнал «нужно сделать», не ошибка.

## Темизация
- Акцентные цвета:
  - `_softRed = Color(0xFFE57373)`
  - `_whatsappGreen = Color(0xFF25D366)`
  - `_telegramBlue = Color(0xFF229ED9)`
- Прозрачности — через новый метод `withValues(alpha: ...)`.

## Производительность
- Размер ячеек вычисляется через `LayoutBuilder`; используется лёгкая разметка `Row`/`SizedBox`.
- Анимации короткие (≈220–250 мс), чтобы не перегружать UI.

## Прогресс по договору (новое)
- Виджет: `ContractProgressWidget`
- Источники данных: суммарные сметы (`estimates`) и выполненные объёмы (`work_items` с join на `estimates.contract_id`).
- Особенности:
  - По умолчанию отображается договор с максимальным процентом выполнения.
  - Переключение между договорами стрелками (влево/вправо).
  - Индикатор: круговой прогресс с кастомной отрисовкой (сегментами) и градиентом от красного → оранжевого → жёлтого → зелёного, утолщённая дуга, «выпуклый» эффект (тени/подсветка).
  - Внутри — процент; ниже — суммы: `Итого смета / Выполнено`.

## Различия по платформам
- Desktop (≥1100px): трёхколоночный layout. Центральная колонка — блок «Нужно согласовать» (если есть), затем «Моя открытая смена», «Сводка смен», «Календарь смен»; правая колонка — прогресс по договору; левая колонка — быстрые переходы, план работ, заявления.
- Mobile/Tablet: вертикальный список: KPI → «Нужно согласовать» (если есть) → открытая смена → быстрые действия → «Сводка смен» → «Календарь смен» → «Прогресс договора» → «План работ» → «Заявления».

## Сводка смен: список сотрудников по наведению (новое)
- Виджет: `HomeShiftsSummaryWidget` (`features/home/presentation/widgets/home_shifts_summary_widget.dart`).
- Источник данных: `ShiftsDataSourceImpl.getShiftsSummaryForDate` (`features/home/data/datasources/shifts_data_source_impl.dart`).
- Запрос к БД: `works → work_hours → employees(first_name, last_name, position)` за выбранную дату.
- По каждому объекту формируются списки ФИО:
  - `itrNames` — сотрудники, не отнесённые к монтажникам (по `position`).
  - `installerNames` — сотрудники с `position`, содержащим «монтажник»/«электрик»/«рабочий».
- UI: бейджи «ИТР» и «Монт.» в строке объекта обёрнуты в `Tooltip` с `richMessage` (список ФИО через маркеры «• »).
  - Подсказка появляется через 300 мс после наведения мыши, держится до 10 секунд.
  - На мобильных срабатывает по долгому нажатию.
  - Если ФИО сотрудников не заполнены — бейдж ведёт себя как раньше (без подсказки).
- Логика отнесения к ИТР/монтажникам не изменилась — по полю `position` сотрудника.

## Связанные файлы
- `features/home/presentation/screens/home_screen.dart` — точка интеграции
- `features/home/presentation/widgets/home_purchase_requests_approval_widget.dart` — заявки на согласование
- `features/home/presentation/widgets/shifts_calendar_widgets.dart` — календарь и flip
- `features/home/presentation/widgets/contract_progress_widget.dart` — прогресс договора
- `features/works/` — модуль с работами и материалами
