...
## 🔗 GitHub MCP Server интеграция (без Docker)

Теперь используется нативный бинарь `github-mcp-server` (Docker не требуется). Репозиторий: [`github/github-mcp-server`](https://github.com/github/github-mcp-server).

Установка бинаря (один из вариантов):
- macOS (Homebrew): `brew install github-mcp-server`
- Manual: скачайте релиз из Releases и добавьте бинарь в PATH

Конфигурация IDE: `.cursor/mcp.json` использует команду `github-mcp-server` и запрашивает только токен.

Требуется персональный токен GitHub (PAT):
- Рекомендуемые скоупы: `repo`, `workflow` (дополнительно `read:org`, если нужно)
- IDE запросит переменную `GITHUB_TOKEN`, она передаётся как `GITHUB_PERSONAL_ACCESS_TOKEN`
- Для GHES задайте `GITHUB_HOST`, например `https://your.ghe.company`

Опционально:
- Включить только чтение: запустить сервер с `--read-only`
- Динамические toolsets: `--dynamic-toolsets`

Примеры возможностей: создание/поиск Issues и PR, просмотр/перезапуск Actions, чтение security advisories (при включённом `code_security`). 