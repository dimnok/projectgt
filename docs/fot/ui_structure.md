...
> **Изменения 09.09.2026:** На вкладке «Премии» — «Импорт из Excel» (`PayrollBonusExcelImportDialog`): дата, объект, примечание → файл `.xlsx` → общий предпросмотр ФИО → `savePayrollBonusBatch`.
...
| Премии | «Импорт из Excel» + «Добавить» | `payroll` / `create` |
...
- **Состав списка:** RPC; при «Все объекты» ещё сотрудники «в штате / с балансом» без начислений; при фильтре объекта — только RPC (`_groupPayrolls`).
...
- Импорт Excel: `PayrollBonusExcelImportDialog` (дата, объект, примечание) → тот же предпросмотр ФИО, что у выплат → `payroll_bonus`. Объект обязателен; если в фильтре выбран один объект, он подставляется. Дата по умолчанию — сегодня при просмотре текущего месяца, иначе 1-е число выбранного месяца.
...
| **Импорт премий** | Панель фильтров, вкладка «Премии» | Тот же парсер ФИО+сумма; запись в `payroll_bonus` |
