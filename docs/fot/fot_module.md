...
**Изменения 09.09.2026:**
- На вкладке «Премии» добавлен клиентский импорт Excel (тот же формат ФИО + сумма, что у выплат). Записи в `payroll_bonus` с обязательным объектом.

**Изменения 28.08.2026:**
- FIFO читает все `payroll_payout` компании порциями по 1000 строк (`getAllPayouts` / `export-payroll`). Один запрос PostgREST обрезал список, старые выплаты не гасили баланс.
- Часы ФОТ и годового PDF идут через загрузку табеля (порции по 1000). Отдельный SELECT `work_hours` в ФОТ убран.

**Изменения в версии аудита 23.08.2026:**
...
- импорт выплат и премий — только клиент (`file_picker` + `excel`), файл на сервер не пишется
...
- `timesheet` — загрузка часов (закрытые смены + ручной табель, пагинация PostgREST); UI-паттерн панели
...
| Добавить премию / штраф / выплату, импорт Excel (выплаты и премии) | `payroll.create` |
...
2. Все `payroll_payout` компании, по дате возрастания. Загрузка порциями: `.order(payout_date).order(id).range` по 1000 строк (лимит PostgREST). То же в `export-payroll`.
