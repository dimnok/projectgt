...
- **Управление доступом (RBAC v3):** Роль (`role_id`), системный статус (`system_role`) и статус активности (`is_active`) теперь хранятся и управляются в таблице `company_members`. Таблица `profiles` хранит только общие метаданные пользователя.
