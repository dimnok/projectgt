...
## Обновление структуры таблиц (2024-05-21)

В таблицы `payroll_bonus`, `payroll_penalty`, `payroll_deduction`, `payroll_payout` добавлено новое поле:

- `employee_id` — UUID, внешний ключ на employees(id), NOT NULL. Теперь каждая запись премии, штрафа, удержания и выплаты напрямую связана с сотрудником.

### Пример структуры (payroll_bonus):
| id | payroll_id | employee_id | type | amount | reason | created_at |
|----|------------|-------------|------|--------|--------|------------|
|UUID|UUID        |UUID         |...   |...     |...     |...         |

### Описание поля:
- `employee_id` — идентификатор сотрудника, которому назначена премия/штраф/удержание/выплата. Позволяет быстро фильтровать и агрегировать данные по сотруднику без дополнительных JOIN.
