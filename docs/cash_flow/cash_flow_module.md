...
- `BankStatementView`: Экран обработки банковской выписки (загрузка файла, таблица записей).
...
- `CashFlowState`: Freezed-класс состояния (статус, списки, фильтры).
  - `searchQuery` — поиск по реестру транзакций (серверный, через `cash_flow_search_text`).
  - `bankStatementSearchQuery` — поиск по записям выписки (клиентский).
  - `filteredBankStatementEntries` — геттер, возвращает `bankStatementEntries`, отфильтрованные по `bankStatementSearchQuery`.
- `setSearchQuery(String)` — перезагружает реестр транзакций с серверным поиском.
- `setBankStatementSearchQuery(String)` — обновляет клиентский фильтр выписки без запроса к БД.
...
- `AvailableFilters`: Доступные ID для контекстной фильтрации (объекты, контрагенты, договоры).
...
- `CashFlowRepository`: Реализация через Supabase. Метод `processBankStatementEntry` передаёт в RPC опциональный `settlementOperationId` → `p_settlement_operation_id`. Метод `getYearlyAnalytics` агрегирует транзакции по месяцам с ключом `year * 100 + month` и возвращает список в хронологическом порядке.
...
6.  **Отображение:** Данные выводятся в `BankStatementTable` через геттер `filteredBankStatementEntries`. Записи, хеш которых уже есть в системе, помечаются как дубликаты.
7.  **Поиск в выписках (клиентский):**
    - Поле поиска в `CashFlowListDesktopView` (подсказка: «Поиск по ИНН или сумме...»).
    - При вводе вызывается `setBankStatementSearchQuery` — без запроса к БД.
    - Поиск по **ИНН контрагента** (`contractor_inn`): полное или частичное совпадение, в т.ч. только цифры.
    - Поиск по **сумме платежа** (`amount`): точное значение, частичное вхождение, форматы `1000`, `1 000,00`, `1000.50`.
    - При переключении между реестром и выпиской поле поиска синхронизируется с соответствующим запросом (`searchQuery` / `bankStatementSearchQuery`).
8.  **Обработка записей:**
...
9.  **Атомарность RPC `process_bank_statement_entry`:**
...
1. Загрузка транзакций за выбранный год с учётом активных фильтров (объект, контрагент, договоры, тип операции, поиск).
2. Группировка по ключу `year * 100 + month` (дата операции в локальном поясе через `toLocal()`).
3. Для UI сохраняется локализованная подпись месяца через `GtFormatters.formatCompactMonthYear` (например, `янв. 2026`).
4. Сортировка колонок — по числовому ключу, **не** через парсинг строки месяца (русские сокращения не парсятся без локали `ru_RU`).
5. Отображение в `CashFlowDetailsPanel` — горизонтально прокручиваемая таблица: ПРИХОД / РАСХОД / ИТОГО (САЛЬДО) по месяцам.
...
- [x] Поиск в банковских выписках по ИНН и сумме платежа.
