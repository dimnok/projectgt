    ...
    private func applyLiquidGlassStyle() {
        ...
        if #available(iOS 15.0, *) {
            ...
            case "plain":
                config = .plain()
            case "glass":
                // Fallback: plain style with background material (iOS 26 API not available in SDK 18.5)
                config = .plain()
                config.background.visualEffect = UIBlurEffect(style: .systemUltraThinMaterial)
            case "prominentGlass":
                // Fallback: tinted style with thicker material (iOS 26 API not available in SDK 18.5)
                config = .tinted()
